...
func (r *PortfolioRepo) GetByUserID(userID int) (*model.Portfolio, error) {
    p := &model.Portfolio{}
    var title, description, category sql.NullString
    err := r.db.QueryRow(
        `SELECT id, user_id, title, description, category, score, created_at
         FROM portfolios WHERE user_id = $1`, userID,
    ).Scan(&p.ID, &p.UserID, &title, &description, &category, &p.Score, &p.CreatedAt)
    if err != nil {
        return nil, err
    }
...
func (r *PortfolioRepo) GetByUsername(username string) (*model.Portfolio, error) {
    ...
    var title, description, category sql.NullString
    err := r.db.QueryRow(
        `SELECT p.id, p.user_id, p.title, p.description, p.category, p.score, p.created_at
         FROM portfolios p
