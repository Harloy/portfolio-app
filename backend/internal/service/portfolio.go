...
func (s *PortfolioService) Save(userID int, title, description, category string, blocks []model.Block) (*model.Portfolio, error) {
    p := &model.Portfolio{
        UserID:      userID,
        Title:       title,
    ...
    if err := s.portfolioRepo.Upsert(p); err != nil {
        return nil, err
    }
    if err := s.blockRepo.ReplaceAll(p.ID, blocks); err != nil {
        return nil, err
    }
    p.Blocks = blocks
    return p, nil
