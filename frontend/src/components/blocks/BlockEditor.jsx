...
import { ContactsEditor } from './ContactsBlock'
import { CommentsEditor } from './CommentsBlock'
import { MailEditor }     from './MailBlock'
...
function CaseEditor({ content, onChange }) {
  let parsed = { title: '', description: '', images: [], link: '', link_text: '' }
  try { parsed = { ...parsed, ...JSON.parse(content || '{}') } } catch {}
  function update(field, value) { onChange(JSON.stringify({ ...parsed, [field]: value })) }
  ...
  return (
    <div className="flex flex-col gap-2">
      <input value={parsed.title || ''} onChange={e => update('title', e.target.value)}
        ...
        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-black transition-colors" />
      <textarea value={parsed.description || ''} onChange={e => update('description', e.target.value)}
      ...
      <div className="flex flex-col gap-1.5">
        {(parsed.images || []).map((img, i) => (
          <div key={i} className="flex gap-2">
            ...
            <button onClick={() => update('images', parsed.images.filter((_, idx) => idx !== i))}
              className="text-gray-300 hover:text-red-400 px-2">✕</button>
          </div>
        ))}
        <button onClick={() => update('images', [...(parsed.images || []), ''])}
          className="text-xs text-gray-400 hover:text-gray-600 text-left py-1">+ добавить картинку</button>
      </div>

      {/* Ссылка */}
      <div className="border-t border-gray-100 pt-2 flex flex-col gap-2">
        <input value={parsed.link || ''} onChange={e => update('link', e.target.value)}
          placeholder="Ссылка на проект (необязательно)"
          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-black transition-colors" />
        {parsed.link && (
          <input value={parsed.link_text || ''} onChange={e => update('link_text', e.target.value)}
            placeholder="Текст ссылки (по умолчанию: Смотреть проект)"
            className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-black transition-colors" />
        )}
      </div>
    </div>
...
export default function BlockEditor({ block, onChange, portfolioId }) {
  switch (block.type) {
    ...
    case 'contacts': return <ContactsEditor content={block.content} onChange={onChange} />
    case 'comments': return <CommentsEditor content={block.content} onChange={onChange} portfolioId={portfolioId} />
    case 'mail':     return <MailEditor     content={block.content} onChange={onChange} />
    default:         return null
  }
}
